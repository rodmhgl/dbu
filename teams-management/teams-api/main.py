...
async def scaffold_workload(team_id: str, workload: WorkloadCreate):
    ...
    image_registry = "ghcr.io/rodmhgl" if workload.language else None
    ...
    if GITHUB_TOKEN and GITHUB_REPO:
        ...
        # Scaffold app source code if language is specified
        if workload.language:
            app_files = scaffold_app_source(
                workload_name=workload_name,
                language=workload.language,
                workload_type=workload.type,
                port=workload.port or 8080,
            )
            app_repo_url = gh.create_repo(
                name=workload_name,
                description=f"{workload_name} — scaffolded by teams-api",
            )
            app_repo = f"rodmhgl/{workload_name}"
            gh.commit_files_to_repo(
                repo=app_repo,
                files=app_files,
                message=f"feat({workload_name}): initial scaffold ({workload.language.value}/{workload.type.value})",
            )

        if workload.language and app_repo_url:
            pr_body = (
                f"## Workload Scaffold\n\n"
                f"- **Team:** {team_name}\n"
                ...
                f"- **Language:** {workload.language.value}\n"
                f"- **App Repo:** {app_repo_url}\n\n"
                f"Generated by teams-api scaffolder.\n\n"
                f"### Next steps\n"
                f"1. Update `commit-sha` annotation with your build SHA\n"
                f"2. Merge to deploy via Flux\n"
            )
        else:
            pr_body = (
                f"## Workload Scaffold\n\n"
